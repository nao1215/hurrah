...
func TestNewFlag(t *testing.T) {
	t.Run("NewFlag without argument. Flag has default value", func(t *testing.T) {
		...
		got := NewFlag()
		want := &Flag{Port: "8080", ConfigFile: "config.toml", Debug: false}
	...
	t.Run("NewFlag with arguments. Flag has the given value", func(t *testing.T) {
		args := []string{"cmd/hurrah/main_test.go", "-port", "1234", "-config", "new.toml", "-debug"}
		...
		got := NewFlag()
		want := &Flag{Port: "1234", ConfigFile: "new.toml", Debug: true}
